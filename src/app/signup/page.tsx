...
const ROLES = [
  ...
  { key: "ngo", icon: "🏢", en: "NGO / Field Agent", bn: "NGO / ফিল্ড এজেন্ট", desc: "Manage farmer clusters" },
  { key: "mfi", icon: "💳", en: "MFI Officer", bn: "MFI কর্মকর্তা", desc: "Credit assessment & loans" },
  { key: "investor", icon: "💰", en: "Investor / Donor", bn: "বিনিয়োগকারী / দাতা", desc: "Track SDG impact & funding" },
...
const DISTRICTS = [
  ...
  "Tangail", "Bogra", "Jessore", "Pabna", "Sirajganj",
];
...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-emerald-50 via-white to-cyan-50 py-10">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-emerald-100 p-8">
          ...
          <div className="mb-6">
            <label className="text-sm font-medium text-gray-600 block mb-3">
              {lang === "en" ? "I am registering as" : "আমি নিবন্ধন করছি"}
            </label>
            ...
            {selectedRole && (
              <p className="text-xs text-emerald-600 mt-2 text-center bg-emerald-50 py-1.5 px-3 rounded-lg">
                {selectedRole.desc}
              </p>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Organization (for NGO/MFI/investor) */}
            {["ngo", "mfi", "investor"].includes(role) && (
              <div>
                <label className="text-sm font-medium text-gray-600 block mb-1">
                  {lang === "en" ? "Organization Name" : "প্রতিষ্ঠানের নাম"}
                </label>
                <input
                  type="text"
            ...
            {role === "ngo" && (
              <div className="bg-amber-50 border border-amber-200 text-amber-700 text-xs px-4 py-3 rounded-xl">
                {lang === "en"
                  ? "NGO accounts require admin approval before activation."
                  : "NGO অ্যাকাউন্ট সক্রিয় হতে অ্যাডমিন অনুমোদন প্রয়োজন।"}
              </div>
            )}
