...
export default function FarmerDashboard() {
  ...
  async function saveMilkLog(e) {
    ...
    try {
      ...
      setMessage('Milk log saved! Total: ' + data.total_yield + 'L');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-cyan-50">
      <header className="bg-white/80 backdrop-blur-md border-b border-emerald-100 sticky top-0 z-50 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => { localStorage.clear(); window.location.href = '/login'; }} className="text-xs text-red-500 hover:underline">{lang === 'en' ? 'Logout' : 'লগআউট'}</button>
          </div>
        </div>
      </header>
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        {tab === 'overview' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white/70 border border-emerald-100 rounded-2xl p-6 shadow-sm">
              ...
              {loading ? <p className="text-gray-400 text-sm">Loading...</p> : milkLogs.length === 0 ? <div className="text-center py-4"><p className="text-gray-400 text-sm">{lang === 'en' ? 'No milk logs yet' : 'এখনো কোন লগ নেই'}</p><button onClick={() => setTab('milk')} className="mt-2 text-xs text-emerald-600 hover:underline">{lang === 'en' ? 'Log first entry →' : 'প্রথম এন্ট্রি →'}</button></div> :
            ...
            <div className="bg-white/70 border border-emerald-100 rounded-2xl p-6 shadow-sm">
              ...
              {cattle.length === 0 ? <div className="text-center py-4"><p className="text-gray-400 text-sm">{lang === 'en' ? 'No cattle registered' : 'কোন গরু নেই'}</p><button onClick={() => setTab('cattle')} className="mt-2 text-xs text-emerald-600 hover:underline">{lang === 'en' ? 'Register cattle →' : 'গরু নিবন্ধন →'}</button></div> :
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white/70 border border-emerald-100 rounded-2xl p-6 shadow-sm">
              ...
              <form onSubmit={saveMilkLog} className="space-y-4">
                ...
                <div className="bg-emerald-50 rounded-xl p-3 text-center"><p className="text-emerald-700 font-bold">{lang === 'en' ? 'Total: ' : 'মোট: '}{((parseFloat(milkForm.morning_yield)||0)+(parseFloat(milkForm.evening_yield)||0)).toFixed(1)}L</p></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white/70 border border-emerald-100 rounded-2xl p-6 shadow-sm">
              ...
              <form onSubmit={saveCattle} className="space-y-4">
                <div><label className="text-sm font-medium text-gray-600 block mb-1">{lang === 'en' ? 'Cow ID / Name' : 'গরুর ID'}</label><input type="text" value={cattleForm.cow_id} onChange={e=>setCattleForm(f=>({...f,cow_id:e.target.value}))} required className={ic} placeholder="COW-001"/></div>
        ...
        {tab === 'credit' && (
          <div className="bg-white/70 border border-emerald-100 rounded-2xl p-6 shadow-sm">
              <div className="text-center py-8"><p className="text-5xl mb-4">📊</p><p className="font-bold text-gray-800">{lang === 'en' ? 'No Credit Score Yet' : 'এখনো স্কোর নেই'}</p><p className="text-gray-500 text-sm mt-2">{lang === 'en' ? 'Register cattle and log milk for 3+ days' : 'গরু নিবন্ধন ও ৩+ দিন দুধ লগ করুন'}</p></div>
            ) : (
